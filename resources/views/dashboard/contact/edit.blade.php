<x-dashboard-layout>
    <div class="p-4">
        <div class="space-y-4 p-4 bg-white rounded-lg max-w-screen-md">
            ...
            <form action="{{ route('contacts.update', ['contact' => $contact]) }}" method="post" class="grid gap-4">
                @csrf
                @method('PUT')
                <label class="block">
                    <span class="label">Nama kontak</span>
                    <input type="text" name="name" class="input" value="{{ $contact->name }}" placeholder="nama sosmed" required>
                    @error('name')
                ...
                <label class="block">
                    ...
                    <input type="number" name="phone" class="input" value="{{ $contact->phone }}" placeholder="nomer" required>
                    @error('phone')
                        <span class="label text-rose-500">{{ $message }}</span>
                    @enderror
                </label>
